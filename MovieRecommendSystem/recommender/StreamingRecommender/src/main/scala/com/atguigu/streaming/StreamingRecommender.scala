...
object ConnHelper extends Serializable {
  lazy val jedis = new Jedis("172.16.104.13")
  lazy val mongoClient = MongoClient(MongoClientURI("mongodb://172.16.104.13:27017/recommender"))
}
...
object StreamingRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val simMoviesMatrix = spark
      ...
      .rdd
      .map { movieRecs =>
        (movieRecs.mid, movieRecs.recs.map(x => (x.rid, x.r)).toMap)
      }.collectAsMap()
    ...
    //创建到Kafka的连接
    val kafkaPara = Map(
      "bootstrap.servers" -> "172.16.104.13:9092",
